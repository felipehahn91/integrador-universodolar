...
serve(async (req) => {
  ...
  const { jobId, full_sync = false } = await req.json();
  if (!jobId) {
  ...
  try {
    ...
    await supabaseAdmin.from('sync_jobs').update({ new_records_added: newContactsCount }).eq('id', jobId);
    await appendLogs(supabaseAdmin, jobId, [`${timestamp()} Passo 1 concluído. ${newContactsCount} novos contatos adicionados.`]);
