...
const Settings = () => {
  ...
  const runJob = async (jobName: string, functionName: string) => {
    ...
    try {
      ...
      const { data: jobData, error: createError } = await supabase.from('sync_jobs').insert({ user_id: user.id, status: 'running', full_sync: true, logs: [`[${new Date().toLocaleTimeString()}] Tarefa manual '${jobName}' iniciada.`] }).select('id').single();
      if (createError) throw createError;
      jobId = jobData.id;

      const { count, error: countError } = await supabase.from('magazord_contacts').select('*', { count: 'exact', head: true });
      ...
      for (let page = 1; page <= totalPages; page++) {
        const { data: jobStatus, error: statusError } = await supabase.from('sync_jobs').select('status').eq('id', jobId).single();
        if (statusError || jobStatus?.status === 'cancelled') {
          toast.info("Tarefa cancelada pelo usuário.", { id: toastId });
          queryClient.invalidateQueries({ queryKey: ['sync_history'] });
          return;
        }

        toast.loading(`Processando ${jobName}: lote ${page} de ${totalPages}...`, { id: toastId });
        const { error: invokeError } = await supabase.functions.invoke(functionName, { body: { page, jobId } });
        if (invokeError) { throw new Error(`Erro no lote ${page}: ${invokeError.message}`); }
      }
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div><h3 className="font-semibold mb-2">Sincronização Histórica com Mautic</h3><p className="text-sm text-muted-foreground mb-3">Enviar toda a base de contatos existente para o Mautic. Use esta opção uma vez para popular o Mautic com dados antigos.</p><Button onClick={() => runJob("Sincronização com Mautic", "mautic-backfill")} disabled={isJobRunning}><RefreshCw className={`mr-2 h-4 w-4 ${isJobRunning ? 'animate-spin' : ''}`} />Sincronizar Base Completa com Mautic</Button></div>
          <div><h3 className="font-semibold mb-2">Atualizar Detalhes dos Contatos</h3><p className="text-sm text-muted-foreground mb-3">Busca os dados mais recentes de todos os contatos, incluindo o telefone. Útil para preencher informações de contatos antigos.</p><Button onClick={() => runJob("Atualização de Contatos", "backfill-contact-details")} disabled={isJobRunning}><RefreshCw className={`mr-2 h-4 w-4 ${isJobRunning ? 'animate-spin' : ''}`} />Atualizar Contatos</Button></div>
          <div><h3 className="font-semibold mb-2">Sincronização Histórica de Pedidos</h3><p className="text-sm text-muted-foreground mb-3">Busca o histórico de pedidos de todos os contatos. Use após importar contatos pela primeira vez.</p><Button onClick={() => runJob("Sincronização de Pedidos", "backfill-orders")} disabled={isJobRunning}><RefreshCw className={`mr-2 h-4 w-4 ${isJobRunning ? 'animate-spin' : ''}`} />Sincronizar Pedidos Históricos</Button></div>
        </CardContent>
      </Card>
    </div>
  );
